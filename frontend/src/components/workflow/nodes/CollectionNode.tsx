...
import { Send, Upload, Settings, AlertCircle } from 'lucide-react';
...
export const CollectionNode: React.FC<NodeProps<CollectionNodeData>> = ({
  data,
  selected,
  id,
}) => {
  const { label, type, status = 'idle', error, config } = data;
  const IconComponent = collectionIcons[type] || Send;
  ...
  const getConfigSummary = () => {
    ...
    switch (type) {
      ...
      case 'bulkImport':
        return config.fileType ? `Type: ${config.fileType.toUpperCase()}` : null;
      default:
        return null;
    }
  };
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-gray-500 mt-1">
            Collection • {type.replace(/([A-Z])/g, ' $1').toLowerCase()}
          </div>
