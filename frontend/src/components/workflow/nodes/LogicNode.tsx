...
import { GitBranch, RotateCcw, Pause, Settings, AlertCircle } from 'lucide-react';
...
export const LogicNode: React.FC<NodeProps<LogicNodeData>> = ({
  data,
  selected,
  id,
}) => {
  const { label, type, status = 'idle', error, config } = data;
  const IconComponent = logicIcons[type] || GitBranch;
  ...
  const getConfigSummary = () => {
    ...
    switch (type) {
      case 'conditional':
        return config.condition ? `If: ${config.condition}` : 'No condition';
      case 'loop':
        return config.itemsPath ? `Loop: ${config.itemsPath}` : 'No loop path';
      case 'wait':
        return config.duration ? `Wait: ${config.duration}s` : 'No duration';
      default:
        return null;
    }
  };
  ...
  const getOutputHandles = () => {
    switch (type) {
      ...
      case 'loop':
        return [
          { id: 'item', label: 'Item', top: '30%' },
          { id: 'completed', label: 'Done', top: '70%' }
        ];
      default:
        return [{ id: 'output', label: '', top: '50%' }];
    }
  };
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-xs text-gray-500 mt-1">
            Logic • {type.replace(/([A-Z])/g, ' $1').toLowerCase()}
          </div>
